...
import com.appciencias.algorithms.ArbolDigital;
import com.appciencias.algorithms.ArbolDigital.Nodo;
...
/**
 * Panel interactivo para Árbol Digital.
 * Permite insertar, buscar y eliminar claves (letras a-z),
 * visualizando el árbol con animaciones.
 */
public class ArbolDigitalPanel extends JPanel {
    ...
    private List<String> animPath = null;     // ruta de nodos a resaltar
    ...
    public ArbolDigitalPanel() {
        arbol = new ArbolDigital();
        initializePanel();
    }
    ...
    private JPanel createHeader() {
        ...
        JLabel titleLabel = new JLabel("Árbol Digital");
    ...
    private void handleInsert() {
        ...
        // Calcular ruta de animación (nodos que se recorren)
        List<String> path = computeInsertPath(clave);

        // Ejecutar inserción real
        try {
        ...
        // Animar el recorrido y resaltar nodo nuevo al final
        runAnimation(path, clave, AnimationType.INSERT, () -> {
            setStatus("Clave '" + clave + "' insertada. Claves en el árbol: " + arbol.getContador(), false);
            treeCanvas.repaint();
        });
    }
    ...
    private void handleSearch() {
        ...
        // Calcular la ruta de búsqueda
        List<String> path = computeSearchPath(clave);

        boolean found = arbol.buscar(clave);

        runAnimation(path, clave, AnimationType.SEARCH, () -> {
            if (found) {
                setStatus("Clave '" + clave + "' encontrada en el árbol.", false);
            } else {
                setStatus("Clave '" + clave + "' NO encontrada en el árbol.", true);
            }
        });
    }
    ...
    private void handleDelete() {
        ...
        // Calcular ruta hasta el nodo a eliminar
        List<String> path = computeSearchPath(clave);

        runAnimation(path, clave, AnimationType.DELETE, () -> {
            try {
                arbol.eliminar(clave);
                setStatus("Clave '" + clave + "' eliminada. Claves en el árbol: " + arbol.getContador(), false);
            } catch (Exception ex) {
                setStatus("Error: " + ex.getMessage(), true);
            }
            treeCanvas.repaint();
    ...
    /**
     * Calcula la lista de claves de nodos visitados al insertar (antes de la inserción real).
     * Recorre el árbol como lo haría insertarEnArbol, recogiendo las claves de los nodos existentes.
     */
    private List<String> computeInsertPath(String clave) {
        List<String> path = new ArrayList<>();
        Nodo raiz = arbol.getRaiz();
        if (raiz == null) {
            // Árbol vacío, no hay camino previo
            return path;
        }
        path.add(raiz.clave);
        ...
        for (int i = 0; i < bits.length(); i++) {
            ...
            if (siguiente == null) {
                break; // aquí se insertará
            }
            path.add(siguiente.clave);
            actual = siguiente;
        }
        return path;
    }
    ...
    /**
     * Calcula la lista de claves visitadas al buscar una clave.
     */
    private List<String> computeSearchPath(String clave) {
        List<String> path = new ArrayList<>();
        Nodo raiz = arbol.getRaiz();
        if (raiz == null) {
            return path;
    ...
    private void runAnimation(List<String> path, String targetKey, AnimationType type, Runnable onFinish) {
        animPath = path;
        animStep = -1;
    ...
    private void finishAnimation() {
        ...
        // Mantener el resaltado final un momento
        treeCanvas.repaint();
    ...
    /**
     * Canvas personalizado que dibuja el árbol digital con nodos circulares,
     * aristas y animaciones. Siempre desplaza hijos izquierda a la izquierda
     * y derecha a la derecha (nunca centrado/recto para hijo único).
     */
    private class TreeCanvas extends JPanel {
        ...
        // Almacena posiciones calculadas para cada nodo
        private Map<String, Point> nodePositions = new HashMap<>();
        // Para nodos con misma clave en distintas posiciones (no aplica aquí, claves únicas)
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Calcular posiciones: primero contamos nodos-hoja para dar espacio
            ...
            // Dibujar aristas primero (detrás de los nodos)
            drawEdges(g2, raiz);
        ...
        /**
         * Calcula el ancho que necesita un subárbol (en píxeles).
         * Cada hoja ocupa al menos MIN_HORIZONTAL_GAP*2.
         * Los nodos sin hijos también necesitan espacio a los lados.
         */
        private int computeSubtreeWidth(Nodo nodo) {
            ...
            // Si solo tiene un hijo, igual reservar espacio para ambos lados
         ...
         * CLAVE: Siempre desplaza hijos a su lado correspondiente, nunca centra un hijo único.
         */
        private void computePositions(Nodo nodo, int x, int y, int halfSpan) {
            ...
            // Siempre dividir espacio en izquierda y derecha
            int leftHalf = halfSpan;
            int rightHalf = halfSpan;

            // Recalcular proporciones si ambos hijos existen
            if (nodo.izquierda != null && nodo.derecha != null) {
        ...
        /**
         * Dibuja las aristas (líneas entre nodos).
         */
        private void drawEdges(Graphics2D g2, Nodo nodo) {
            ...
            if (nodo.izquierda != null) {
                ...
                if (childPos != null) {
                    boolean highlighted = isEdgeHighlighted(nodo.clave, nodo.izquierda.clave);
            ...
            if (nodo.derecha != null) {
                ...
                if (childPos != null) {
                    boolean highlighted = isEdgeHighlighted(nodo.clave, nodo.derecha.clave);
                    g2.setColor(highlighted ? getAnimColor() : EDGE_COLOR);
                    g2.setStroke(new BasicStroke(highlighted ? 3f : 2f));
                    g2.drawLine(parentPos.x, parentPos.y + NODE_RADIUS,
                            childPos.x, childPos.y - NODE_RADIUS);
        ...
        /**
         * Dibuja los nodos recursivamente.
         */
        private void drawNodes(Graphics2D g2, Nodo nodo) {
            ...
            // Borde
            boolean highlighted = isNodeHighlighted(nodo.clave);
        ...
        /**
         * Dibuja una leyenda en la esquina.
         */
        ...
        /**
         * Determina el color de relleno de un nodo, considerando la animación.
         */
        private Color getNodeFillColor(Nodo nodo) {
            if (!animationRunning || animPath == null || nodo.clave == null) {
                return NODE_COLOR;
            }

            // ¿Es el nodo actual en la animación?
            if (animStep >= 0 && animStep < animPath.size()) {
                String stepKey = animPath.get(animStep);
                if (nodo.clave.equals(stepKey)) {
                    // Nodo actualmente siendo visitado
                    return getAnimColor();
                }
            }

            // ¿Es un nodo ya visitado en la animación?
            for (int i = 0; i < animStep && i < animPath.size(); i++) {
                if (nodo.clave.equals(animPath.get(i))) {
                    return NODE_HIGHLIGHT_COLOR;
                }
            }

            // Si la animación terminó y es el nodo target
            if (animStep >= animPath.size() && nodo.clave.equals(animTargetKey)) {
                switch (animType) {
                    case INSERT:
        ...
        private boolean isNodeHighlighted(String clave) {
            if (!animationRunning || animPath == null || clave == null) {
                return false;
            }
            if (animStep >= 0 && animStep < animPath.size()) {
                return clave.equals(animPath.get(animStep));
            }
            if (animStep >= animPath.size()) {
                return clave.equals(animTargetKey);
            }
            return false;
        }

        private boolean isEdgeHighlighted(String parentKey, String childKey) {
            if (!animationRunning || animPath == null || parentKey == null || childKey == null) {
                return false;
            }
            for (int i = 0; i < animStep && i + 1 < animPath.size(); i++) {
                if (animPath.get(i).equals(parentKey) && animPath.get(i + 1).equals(childKey)) {
                    return true;
                }
            }
            if (animStep > 0 && animStep < animPath.size()) {
                if (animPath.get(animStep - 1).equals(parentKey) && animPath.get(animStep).equals(childKey)) {
                    return true;
                }
            }
            return false;
        ...
        /**
         * Genera un ID único por instancia de nodo (usando identityHashCode
         * para distinguir nodos aunque tuvieran la misma clave, lo cual no
         * sucede en ArbolDigital, pero es defensivo).
         */
        ...
        private int getLeafCount(Nodo nodo) {
            ...
            // Si solo tiene un hijo, contar al menos 1 para cada lado
            if (nodo.izquierda == null) {
                left = 1;
            }
            if (nodo.derecha == null) {
