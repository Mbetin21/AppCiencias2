 ...
 *
 * El usuario elige el nodo origen y el nodo destino.
 * Si el origen no es el nodo ordinal 1, se ignoran todos los nodos
 * con ordinal menor al origen y se arranca Bellman desde ahi.
 *
 * Algoritmo:
 *   1. Aplicar numeracion ordinal al grafo completo.
 *   2. Tomar como punto de partida el nodo indicado por el usuario.
 *      λ_origen = 0. Los nodos anteriores en ordinal se descartan.
 *   3. Para cada nodo j en orden ordinal desde el origen hasta el destino:
 *        λj = min( λi + θij ) para todo i predecesor de j ya calculado.
 *   4. Reconstruir el camino minimo desde origen hasta destino
...
public class Bellman {
     ...
     *
     * Si el origen no es el primero en ordinal, los nodos anteriores
     * se ignoran y el algoritmo arranca desde el origen indicado.
     *
     ...
     * @throws IllegalArgumentException si el grafo esta vacio, tiene ciclos,
     *                                  o los nodos no existen o el origen tiene
     *                                  ordinal mayor o igual al destino.
     */
    public static Resultado calcular(GrafoPonderado grafo,
        ...
        for (int ord = ordinalOrigen + 1; ord <= ordinalDestino; ord++) {
            ...
            for (GrafoPonderado.AristaPonderada arista : aristas) {
                if (!arista.destino.equals(vj)) continue;

                String vi = arista.origen;

                // Solo predecesores dentro del rango calculado
                if (!lambdas.containsKey(vi)) continue;
            ...
            if (mejorPrec != null) predecesor.put(vj, mejorPrec);
